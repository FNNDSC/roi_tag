...
'''
    ...
    'roi_ROIcontroller_intragroup_sample.py' resamples a homogenous population into
    two separate subgroups and then runs a model-free cloud based
    analysis on the resultant two groups.

    It is used to determine how "homogenous" a population really is.
...
class FNNDSC_ROIINTRAGROUP(base.FNNDSC):
    '''
...
def synopsis(ab_shortOnly = False):
    ...
    DESCRIPTION
        ...
        `%s' explores the homogeneity of a single population sample. The
        process divides its input <subjs> into two random groups and then
        performs a cloud analysis on these two groups.

        In the case for a completely homogeneous sample set, such a reordering
        shouldn't result in statistically (or geometrically) separate groups.
...
if __name__ == "__main__":
    ...
    parser.add_argument('--output', '-o',
                        dest='outDir',
                        action='store',
                        default='ROIcontroller_intragroup',
    ...
    ROIcontroller_intragroup = FNNDSC_ROIINTRAGROUP(
                        ...
                        logTo                   = 'ROIcontroller_intragroup.log',
                        syslog                  = True,
                        logTee                  = True)

    ROIcontroller_intragroup.verbosity(args.verbosity)
    pipeline    = ROIcontroller_intragroup.pipeline()
    pipeline.poststdout(True)
    pipeline.poststderr(True)
    ROIcontroller_intragroup.topDir(os.getcwd())

    stage0 = stage.Stage(
                        name            = 'ROIcontroller_intragroup-0-init',
                        fatalConditions = True,
                        syslog          = True,
                        logTo           = 'ROIcontroller_intragroup-0-init.log',
                        logTee          = True,
                        )
    def f_stage0callback(**kwargs):
        ...
        stage.exitCode(0)
        return True
    stage0.def_stage(f_stage0callback, obj=stage0, pipe=ROIcontroller_intragroup)

    stage1 = stage.Stage(
                        name            = 'ROIcontroller_intragroup-1-resample',
                        fatalConditions = True,
                        syslog          = True,
                        logTo           = 'ROIcontroller_intragroup-1-resample.log',
                        logTee          = True
                        )
    ...
    def f_stage1callback(**kwargs):
        ...
        for pipeline._str_annotation in pipeline.l_annotation():
            for pipeline._str_subsample in pipeline.l_subsample():
                ...
                str_execCmd = "%s -o %s -r %s -s %s -a %s %s" % \
                    (
                        "/neuro/users/rudolphpienaar/src/devel/roi_tag/intra_rsample.py",
                        str_outDir,
        ...
        stage.exitCode(0)
        return True
    stage1.def_stage(f_stage1callback, obj=stage1, pipe=ROIcontroller_intragroup)

    stage2 = stage.Stage(
                        name            = 'ROIcontroller_intragroup-2-curvatureAnalysis',
                        fatalConditions = True,
                        syslog          = True,
                        logTo           = 'ROIcontroller_intragroup-2-curvatureAnalysis.log',
                        logTee          = True
                        )
    ...
    def f_stage2callback(**kwargs):
        ...
        stage.exitCode(0)
        return True
    stage2.def_stage(f_stage2callback, obj=stage2, pipe=ROIcontroller_intragroup)


    stage3 = stage.Stage(
                        name            = 'ROIcontroller_intragroup-3-dty_analysis',
                        fatalConditions = True,
                        syslog          = True,
                        logTo           = 'ROIcontroller_intragroup-3-dty_analysis.log',
                        logTee          = True
                        )
    ...
    def f_stage3callback(**kwargs):
        ...
        stage.exitCode(0)
        return True
    stage3.def_stage(f_stage3callback, obj=stage3, pipe=ROIcontroller_intragroup)


    stage4 = stage.Stage(
                        name            = 'ROIcontroller_intragroup-4-roi_tag',
                        fatalConditions = True,
                        syslog          = True,
                        logTo           = 'ROIcontroller_intragroup-4-roi_tag.log',
                        logTee          = True
                        )
...
#                        pipeline.topDir(), str_outDir,
        ...
        stage.exitCode(0)
        return True
    stage4.def_stage(f_stage4callback, obj=stage4, pipe=ROIcontroller_intragroup)


    ROIcontroller_intragrouplog = ROIcontroller_intragroup.log()
    ROIcontroller_intragrouplog('INIT: %s\n' % ' '.join(sys.argv))
    ROIcontroller_intragroup.stage_add(stage0)
    ROIcontroller_intragroup.stage_add(stage1)
    ROIcontroller_intragroup.stage_add(stage2)
    ROIcontroller_intragroup.stage_add(stage3)
    ROIcontroller_intragroup.stage_add(stage4)
    ROIcontroller_intragroup.initialize()


    ROIcontroller_intragroup.run()
